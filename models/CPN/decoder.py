...
class DepthDecoder(nn.Module):
    ...
    def forward(self, input_features, feature_mask):
        ...
        cum_mask = torch.cumsum(feature_mask, dim=1)   
        inpaint_mask = torch.cat([torch.zeros_like(cum_mask[:, -1:, :, :]), cum_mask[:, :-1, :, :]], dim=1)   
        context_mask = 1 - inpaint_mask   
        ...
        conv_up2 = torch.cat([conv_up2 * cur_context_mask, cur_context_mask, cur_feature_mask], dim=2)   
        conv_up2 = conv_up2.reshape(-1, C_feat + 2, H_feat, W_feat)   
        ...
        for i, feat in enumerate(input_features):
            ...
            feat = torch.cat([feat * cur_context_mask, cur_context_mask, cur_feature_mask], dim=2)   
            input_features[i] = feat.reshape(-1, C_feat + 2, H_feat, W_feat)   
