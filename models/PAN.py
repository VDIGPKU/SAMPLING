...
class MultiheadSelfAttention(nn.Module):
    ...
    def forward(self, feat):
        ...
        return self.fusion(torch.cat(feat_atted, dim=-1))   
...
class DepthPredictionNetwork(nn.Module):
    ...
    def forward(self, init_disp, rgb_low_res, disp_low_res):
        ...
        context = F.adaptive_avg_pool2d(context, (1, 1)).squeeze(-1).squeeze(-1)   
        context = context.view(B, S, -1)   

         
        feat_atted = self.self_attention(context)   
        feat = self.embed(feat_atted)   
        disp_bs = self.to_disp(feat, init_disp)   
        return disp_bs